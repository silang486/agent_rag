...
【历史反馈】：
{feedback}
(如果有反馈意见，请务必根据意见修正你的回答；如果没有，请忽略。)
...
请按照以下标准审核：
1. **事实准确性**：核心技术概念不能出错（例如Agent架构的组成）。
2. **教学灵活性**：如果老师用了类比或者其他的修辞手法（比如用做饭比喻AI），只要比喻逻辑通顺，**应该允许甚至表扬**，不要因为教材里没写红烧肉就驳回。
3. **推论合理性**：如果老师基于教材做了合理的逻辑延伸（比如提到循环交互），只要不违背原意，应当通过。

输出格式：
- 如果回答合格，请仅输出一行：【通过】
- 如果需要修改，请输出：【驳回】 + 具体的修改建议（指出哪里错了，怎么改）。
""")
